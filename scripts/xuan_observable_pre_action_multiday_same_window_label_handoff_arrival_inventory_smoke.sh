...
out_dir="${1:-$ROOT/xuan_research_artifacts/xuan_observable_pre_action_multiday_same_window_label_handoff_arrival_inventory_smoke_$ts}"
mkdir -p "$out_dir"
log="$out_dir/smoke.log"
: > "$log"

export PYTHONPYCACHEPREFIX="${PYTHONPYCACHEPREFIX:-${TMPDIR:-/tmp}/xuan_multiday_label_handoff_arrival_inventory_pycache_$ts}"
trap 'rm -rf "$PYTHONPYCACHEPREFIX"' EXIT

python3 -m py_compile scripts/xuan_observable_pre_action_multiday_same_window_label_handoff_arrival_inventory.py >> "$log" 2>&1
...
script = Path("scripts/xuan_observable_pre_action_multiday_same_window_label_handoff_arrival_inventory.py")
...
def joined_row(handoff_id, day, idx, *, candidate_id=None, action_id=None, forbidden_live=False, missing_candidate=False):
    admitted = idx % 2 == 0
    return {
        "schema_version": "observable_pre_action_training_bridge_joined_row_v1",
        "bridge_row_id": f"{handoff_id}-bridge-{idx}",
        "source_seed_candidate_row_id": "" if missing_candidate else (candidate_id or f"{handoff_id}-candidate-{idx}"),
...
def materialize_source(
    ...
    *,
    ...
    write_rows=True,
    count_delta=0,
    missing_handoff_id=False,
    missing_candidate=False,
    materialized=True,
):
    handoff_id = handoff_id or f"handoff-{day}"
    source_dir = case_dir / (source_dir_name or handoff_id)
    ...
    summary = {
        ...
        "source_handoff_id": "" if missing_handoff_id else handoff_id,
...
def make_case(name, days, rows_per_day=40, **kwargs):
    ...
    for index, day in enumerate(days):
        duplicate_candidate = f"handoff-{days[0]}-candidate-0" if kwargs.get("duplicate_candidate") and index == 1 else None
        duplicate_action = f"handoff-{days[0]}-action-0" if kwargs.get("duplicate_action") and index == 1 else None
        handoff_id = "duplicate-handoff" if kwargs.get("duplicate_handoff") and index < 2 else None
        source_dir_name = f"duplicate-handoff-source-{index}" if handoff_id else None
        summaries.append(
            materialize_source(
                case_dir,
                day,
                ...
                missing_handoff_id=kwargs.get("missing_handoff_id", False) and index == 0,
                missing_candidate=kwargs.get("missing_candidate", False) and index == 0,
...
cases = {
    ...
    "missing_exact_ids": run_case(
        "missing_exact_ids",
        make_case("missing_exact_ids", ["2026-05-26"], rows_per_day=120, missing_handoff_id=True, missing_candidate=True),
    ),
...
checks = {
    ...
    == "KEEP_OBSERVABLE_PRE_ACTION_MULTIDAY_SAME_WINDOW_LABEL_HANDOFF_ARRIVAL_INVENTORY_READY",
    "complete_accumulator_ready": cases["complete"]["readiness"]["accumulator_input_ready"] is True,
    "complete_named_accumulator_command": "--bridge-summary" in cases["complete"]["validation_commands"][0],
    "single_day_unknown": cases["single_day"]["decision_label"]
    == "UNKNOWN_OBSERVABLE_PRE_ACTION_MULTIDAY_SAME_WINDOW_LABEL_HANDOFF_ARRIVAL_INVENTORY_GAPS",
    "missing_exact_ids_unknown": cases["missing_exact_ids"]["decision_label"]
    == "UNKNOWN_OBSERVABLE_PRE_ACTION_MULTIDAY_SAME_WINDOW_LABEL_HANDOFF_ARRIVAL_INVENTORY_GAPS"
    and cases["missing_exact_ids"]["score"]["candidate_exact_id_missing_count"] == 120,
    ...
    == "DISCARD_OBSERVABLE_PRE_ACTION_MULTIDAY_SAME_WINDOW_LABEL_HANDOFF_ARRIVAL_INVENTORY_FORBIDDEN_INPUT",
...
manifest = {
    "artifact": "xuan_observable_pre_action_multiday_same_window_label_handoff_arrival_inventory_smoke",
    "case_decisions": {name: data["decision_label"] for name, data in cases.items()},
    "checks": checks,
    "decision": "KEEP" if all(checks.values()) else "BLOCKED",
    "decision_label": "KEEP_OBSERVABLE_PRE_ACTION_MULTIDAY_SAME_WINDOW_LABEL_HANDOFF_ARRIVAL_INVENTORY_SMOKE_PASS"
    if all(checks.values())
    else "BLOCKED_OBSERVABLE_PRE_ACTION_MULTIDAY_SAME_WINDOW_LABEL_HANDOFF_ARRIVAL_INVENTORY_SMOKE_FAIL",
    "strategy_evidence": False,
