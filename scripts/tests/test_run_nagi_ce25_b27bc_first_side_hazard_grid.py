...
def load_module():
    ...
    module_path = scripts_dir / "run_nagi_ce25_b27bc_first_side_hazard_grid.py"
    spec = importlib.util.spec_from_file_location(
        "run_nagi_ce25_b27bc_first_side_hazard_grid", module_path
    )
...
class NagiCe25B27bcFirstSideHazardGridTests(unittest.TestCase):
    def test_grid_cli_writes_decision_register_without_claims(self):
        with tempfile.TemporaryDirectory(prefix="nagi_first_side_grid_") as tmp:
            root = Path(tmp)
