#!/usr/bin/env python3
"""Strict-L1 clip x l1-cap sweep for xuan taker-BUY research.

This is a causal replay audit:
- L1 uses exact latest recv_ms <= trade_ts_ms.
- L2 still prices first-leg VWAP and completion within the configured window.
- SQLite replay is read-only; outputs are JSON only.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--clips", default="10,20,30,45,60")
    parser.add_argument("--l1-caps", default="0.99,1.0,1.005,1.01,1.02")
...
def metric_key(clip: float, cap: float) -> str:
    return f"clip={clip:g}|l1={cap:g}"
...
def finalize_with_params(metric: dict[str, Any], market_total: int, clip: float, cap: float) -> dict[str, Any]:
...
def run_day(day: str, clips: list[float], caps: list[float], args: argparse.Namespace) -> dict[str, Any]:
    db_path = args.replay_root / day / "crypto_5m.sqlite"
    t0 = time.time()
    metrics = {(clip, cap): metric_template(clip) for clip in clips for cap in caps}
    with ro_connect(db_path) as conn:
        markets = load_markets_official(conn)
        print(f"{day}: markets={len(markets)} strict_l1_clip_cap_sweep", flush=True)
        for idx, market in enumerate(markets, start=1):
            ...
            states = {
                (clip, cap): {"entries": 0, "residuals": 0, "active_until": -1}
                for clip in clips
                for cap in caps
            }
            for trade in trades:
                ...
                for clip in clips:
                    ...
                    if first_vwap is None:
                        for cap in caps:
                            metrics[(clip, cap)]["gates"]["first_l2_depth_or_age"] += 1
                        continue
                    if not (0.50 <= first_vwap <= 0.80):
                        for cap in caps:
                            metrics[(clip, cap)]["gates"]["vwap"] += 1
                        continue
                    immediate_pair = first_vwap + float(opp_ask)
                    completion: dict[str, Any] | None = None
                    for cap in caps:
                        metric = metrics[(clip, cap)]
                        state = states[(clip, cap)]
    ...
    return {
        ...
        "summaries": {
            metric_key(clip, cap): finalize_with_params(metrics[(clip, cap)], len(markets), clip, cap)
            for clip in clips
            for cap in caps
        },
    }
...
def combine_days(day_results: list[dict[str, Any]], clips: list[float], caps: list[float]) -> dict[str, Any]:
    market_total = sum(int(result["market_total"]) for result in day_results)
    combined = []
    for clip in clips:
        for cap in caps:
            merged = metric_template(clip)
            key = metric_key(clip, cap)
            ...
            combined.append(finalize_with_params(merged, market_total, clip, cap))
    return {
        "scope": "BTC 5m strict-L1 clip x l1-cap sweep, official settled only, read-only /mnt/poly-replay",
        ...
        "daily": {
            result["day"]: {
                key: {k: v for k, v in summary.items() if k != "pnl_seq"}
...
def main() -> int:
    ...
    for day in days:
        ...
        if day_path.exists() and not args.force:
            ...
            result = run_day(day, clips, caps, args)
            write_json(day_path, result)
            print(f"{day}: wrote {day_path} elapsed={result['elapsed_sec']}s", flush=True)
        day_results.append(result)
        write_json(args.output_dir / "combined_partial.json", combine_days(day_results, clips, caps))
    combined = combine_days(day_results, clips, caps)
