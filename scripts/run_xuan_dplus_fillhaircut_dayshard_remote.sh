...
BASE="${1:-/home/ubuntu/xuan_frontier_runs/d_branch_fillhaircut_calibration_full_dayshard_0502_0512_20260514_0155}"
...
PY
  ...
  rm -rf "$OUT"
  mkdir -p "$OUT" "/home/ubuntu/duckdb_tmp_xuan_d_branch_dayshard_$TAG"
  echo "$(date -Is) running fill_haircut=$FH out=$OUT"
  nice -n 5 "$PY" "$SCRIPT" \
    "${COMMON_STORES[@]}" \
    --out-dir "$OUT" \
    --tmp-dir "/home/ubuntu/duckdb_tmp_xuan_d_branch_dayshard_$TAG" \
    --edges 0.04 \
    --targets 5 \
    ...
    --imbalance-qty-caps 2 \
    --salvage-net-caps 0.93 0.95 0.96 \
    --salvage-age-s 30 \
...
base = pathlib.Path("/home/ubuntu/xuan_frontier_runs/d_branch_fillhaircut_calibration_full_dayshard_0502_0512_20260514_0155")
rows = []
...
(base / "fillhaircut_full_summary.json").write_text(json.dumps(rows, indent=2))
with (base / "fillhaircut_full_summary.csv").open("w", newline="") as handle:
    writer = csv.DictWriter(handle, fieldnames=list(rows[0].keys()))
    writer.writeheader()
    writer.writerows(rows)
print(json.dumps({"out": str(base), "top": rows[:10]}, indent=2))
PY
