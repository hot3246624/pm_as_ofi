...
out_dir="${1:-$ROOT/xuan_research_artifacts/xuan_b27_dplus_ec2_30m_acceptance_smoke_$ts}"
mkdir -p "$out_dir"
...
latest_review="$(ls -1dt "$ROOT"/xuan_research_artifacts/xuan_b27_dplus_ec2_readonly_user_ws_acceptance_*/local_acceptance_review.json 2>/dev/null | head -1 || true)"

run_check "latest_acceptance_review_exists" test -n "$latest_review"
run_check "latest_acceptance_review_safe_and_passed" python3 - "$latest_review" <<'PY'
import json
import pathlib
import sys
...
ok = (
    data.get("artifact") == "xuan_b27_dplus_ec2_readonly_user_ws_acceptance_local_acceptance_review"
    and data.get("status") == "PASS_READONLY_USER_WS_ACCEPTANCE"
    ...
    and data.get("duration_secs") == 1800
    and data.get("elapsed_ms", 0) >= 1_790_000
    and data.get("shared_ingress_status") == "OK"
    and data.get("broker_preflight_status") == "OK"
...
cat > "$out_dir/manifest.json" <<JSON
{
  ...
  "scope": "local_no_network_ec2_30m_acceptance_gate",
  "acceptance_review": "$latest_review",
  "orders_sent": false,
