#!/usr/bin/env python3
"""Probe public-trade trigger features under candidate-stable D+ metrics."""
...
ARTIFACT = "xuan_b27_dplus_candidate_stable_public_trade_probe"
...
def first_vs_public_trade_slippage(row: dict[str, Any]) -> float | None:
    first_price = row.get("first_price")
    public_trade_price = row.get("public_trade_price")
    if first_price is None or public_trade_price is None:
        return None
    return float(first_price) - float(public_trade_price)


def row_passes(
    row: dict[str, Any],
    public_trade_price_max: float,
    public_trade_size_max: float,
    slippage_max: float,
) -> bool:
    public_trade_price = row.get("public_trade_price")
    public_trade_size = row.get("public_trade_size")
    slippage = first_vs_public_trade_slippage(row)
    if public_trade_price_max > 0:
        if public_trade_price is None or float(public_trade_price) > public_trade_price_max:
            return False
    if public_trade_size_max > 0:
        if public_trade_size is None or float(public_trade_size) > public_trade_size_max:
            return False
    if slippage_max < 99:
        if slippage is None or slippage > slippage_max:
            return False
    return True
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--public-trade-price-maxes", default="0.50,0.55,0.60,0.70")
    parser.add_argument("--public-trade-size-maxes", default="0,100,125,150,200")
    parser.add_argument(
        "--slippage-maxes",
        default="99,-0.05,-0.02,0",
        help="99 disables the slippage filter; otherwise first_price-public_trade_price <= value.",
    )
...
def main() -> int:
    ...
    if input_safe:
        for first_max_price, pair_cap in parse_configs(args.configs):
            ...
            for price_max in parse_csv_floats(args.public_trade_price_maxes):
                for size_max in parse_csv_floats(args.public_trade_size_maxes):
                    for slippage_max in parse_csv_floats(args.slippage_maxes):
                        filtered = [
                            row
                            for row in base_rows
                            if row_passes(row, price_max, size_max, slippage_max)
                        ]
                        if not filtered:
                            continue
                        runs.append(
                            {
                                "kind": "candidate_stable_public_trade_run",
                                "config": {
                                    **base_config,
                                    "public_trade_price_max": price_max,
                                    "public_trade_size_max": size_max,
                                    "public_trade_size_filter_enabled": size_max > 0,
                                    "slippage_max": slippage_max,
                                    "slippage_filter_enabled": slippage_max < 99,
                                },
                                "metrics": summarize_rows(filtered, args.fee_rate),
                            }
                        )
    ...
    metrics_path = output_dir / "public_trade_runs.jsonl"
    metrics_path.write_text("".join(json.dumps(row, sort_keys=True) + "\n" for row in top_rows))
    status = (
        "BLOCKED_PUBLIC_TRADE_PROBE_INPUT_GAP"
        if not input_safe
        else "KEEP_QUALIFIED_PUBLIC_TRADE_CANDIDATE"
        if qualified
        else "DISCARD_NO_QUALIFIED_PUBLIC_TRADE_FILTER"
    )
    manifest = {
        "schema_version": 1,
        "artifact": ARTIFACT,
        ...
        "scope": "local_no_network_candidate_stable_public_trade_probe",
        "status": status,
        "hypothesis": (
            "public trade trigger price/size/slippage can isolate a deployable low-residual seed family "
            "under candidate-stable strict-first metrics"
        ),
        ...
        "best_run": best,
        "public_trade_runs_jsonl": str(metrics_path),
        ...
        "next_action": (
            "parameterize and retest the qualified public-trade filter"
            if qualified
            else "kill public-trade price/slippage filters as a shadow candidate; move to a different signal family"
        ),
    }
