#!/usr/bin/env python3
"""Score source-opportunity ledger-before/delta marker summaries.
...
REQUIRED_LEDGER_BEFORE_DELTA_FIELDS = (
    "schema_version",
    "field_contract",
...
MERGE_FIELDS = tuple(field for field in REQUIRED_LEDGER_BEFORE_DELTA_FIELDS if field not in {"schema_version", "field_contract"})
...
def marker_key_matches(marker_key: str, args: argparse.Namespace, marker_kind: str) -> bool:
    parts = marker_key.split("|")
    if len(parts) != 6:
        return False
    side, offset, risk, open_value, deficit, ledger_bucket = parts
    requested_bucket = args.ledger_before_bucket if marker_kind == "before" else args.ledger_delta_bucket
    terms = {
        "side": side,
        "offset": offset,
        "risk": risk,
    ...
    requested = {
        ...
        "ledger_bucket": requested_bucket,
    }
    return all(value == "ANY" or terms.get(field) == value for field, value in requested.items())


def status_reason_marker_matches(status_reason_marker: str, args: argparse.Namespace, marker_kind: str) -> bool:
    parts = status_reason_marker.split("|")
    if len(parts) != 8:
        return False
    return marker_key_matches("|".join(parts[2:]), args, marker_kind)
...
def marker_denominator(diag: dict[str, Any], args: argparse.Namespace, marker_kind: str) -> dict[str, Any]:
    field = f"transition_count_by_status_side_offset_risk_open_deficit_ledger_{marker_kind}"
    ...
    for row in flatten_status_counts(by_status):
        if not marker_key_matches(row["marker_key"], args, marker_kind):
            continue
        status = row["status"]
    ...
    return {
        "selected_predicate": {
            ...
            "open": args.open_bucket,
            "deficit": args.deficit_bucket,
            f"ledger_{marker_kind}": args.ledger_before_bucket if marker_kind == "before" else args.ledger_delta_bucket,
        },
...
def exact_reason_source_coverage(diag: dict[str, Any], args: argparse.Namespace, marker_kind: str) -> dict[str, Any]:
    count_field = f"transition_count_by_status_reason_side_offset_risk_open_deficit_ledger_{marker_kind}"
    ...
    for status_reason_marker, value in count_table.items():
        ...
        if not status_reason_marker_matches(key, args, marker_kind):
            continue
        ...
        rows.append(
            {
                ...
                "quote_intent_presence_rate": round(
                    source_presence_rate(
                        diag,
                        f"quote_intent_presence_by_status_reason_side_offset_risk_open_deficit_ledger_{marker_kind}",
                        key,
                    ),
                    8,
                ),
                "source_order_presence_rate": round(
                    source_presence_rate(
                        diag,
                        f"source_order_presence_by_status_reason_side_offset_risk_open_deficit_ledger_{marker_kind}",
                        key,
                    ),
                    8,
                ),
                "source_sequence_presence_rate": round(
                    source_presence_rate(
                        diag,
                        f"source_sequence_presence_by_status_reason_side_offset_risk_open_deficit_ledger_{marker_kind}",
                        key,
                    ),
                    8,
                ),
...
def build_score(args: argparse.Namespace) -> dict[str, Any]:
    ...
    missing_fields = [field for field in REQUIRED_LEDGER_BEFORE_DELTA_FIELDS if field not in aggregate_diag and field not in merged_diag]
    ...
    before_delta_schema_ok = bool(
        isinstance(field_contract, dict)
        and field_contract.get("ledger_before_delta_marker_schema_version")
    ...
    before_field_present = "ledger_proxy_before_bucket" in live_pre_action_fields
    delta_field_present = "ledger_proxy_delta_bucket_when_available" in live_pre_action_fields

    before = marker_denominator(diag_for_scoring, args, "before")
    delta = marker_denominator(diag_for_scoring, args, "delta")
    before_reason_source = exact_reason_source_coverage(diag_for_scoring, args, "before")
    delta_reason_source = exact_reason_source_coverage(diag_for_scoring, args, "delta")
    ...
    if missing_fields:
        blockers.append("required_source_opportunity_ledger_before_delta_marker_fields_missing")
    if aggregate_parity_diffs:
        blockers.append("aggregate_parity_failed")
    if not before_delta_schema_ok:
        blockers.append("ledger_before_delta_marker_schema_missing")
    if not before_field_present:
        blockers.append("ledger_before_pre_action_field_not_declared")
    if not delta_field_present:
        blockers.append("ledger_delta_pre_action_field_not_declared")
    ...
    if before["marker_total"] < args.min_before_marker_count:
        blockers.append("selected_ledger_before_marker_denominator_below_threshold")
    if delta["marker_total"] < args.min_delta_marker_count:
        blockers.append("selected_ledger_delta_marker_denominator_below_threshold")
    if before_reason_source["available"] and before_reason_source["exact_reason_marker_total"] < args.min_before_marker_count:
        blockers.append("selected_ledger_before_reason_marker_denominator_below_threshold")
    if delta_reason_source["available"] and delta_reason_source["exact_reason_marker_total"] < args.min_delta_marker_count:
        blockers.append("selected_ledger_delta_reason_marker_denominator_below_threshold")
    if args.require_risk_budget and not risk_budget["passed"]:
        blockers.append("normalized_risk_budget_not_passed")

    if blockers:
        decision = "UNKNOWN"
        label = "UNKNOWN_SOURCE_OPPORTUNITY_LEDGER_BEFORE_DELTA_MARKER_SUMMARY_INPUTS_INSUFFICIENT"
        next_action = (
            "Do not enable any ledger guard or canary. Use blockers to decide whether before/delta "
            "denominators are absent, marker reproduction is thin, or risk budget failed."
        )
    else:
        decision = "KEEP"
        label = "KEEP_SOURCE_OPPORTUNITY_LEDGER_BEFORE_DELTA_MARKER_DENOMINATOR_SCORER_READY"
        next_action = (
            "Use this scorer after an approved bounded no-order pullback before considering another local "
            "target review. This is not private truth, deployable evidence, canary evidence, or promotion."
        )

    return {
        "artifact": "xuan_b27_dplus_source_opportunity_ledger_before_delta_marker_summary_scorer",
        ...
        "contract": {
            ...
            "ledger_before_delta_marker_schema_version": (
                field_contract.get("ledger_before_delta_marker_schema_version") if isinstance(field_contract, dict) else None
            ),
            "selected_before_predicate": before["selected_predicate"],
            "selected_delta_predicate": delta["selected_predicate"],
            "source_pair_source_residual_used_as_live_criteria": False,
            "post_action_outcome_labels_included": False,
            "reason_join_limitation": (
                "The before/delta marker summary reports presence buckets for quote/order/source_sequence, "
                "not raw ids or private/order truth. It is denominator instrumentation only."
            ),
        },
        ...
        "field_checks": {
            ...
            "before_delta_schema_ok": before_delta_schema_ok,
            "ledger_before_pre_action_field_declared": before_field_present,
            "ledger_delta_pre_action_field_declared": delta_field_present,
        ...
        "ledger_before_marker_denominator": before,
        "ledger_delta_marker_denominator": delta,
        "ledger_before_reason_source_coverage": before_reason_source,
        "ledger_delta_reason_source_coverage": delta_reason_source,
        "thresholds": {
            "min_before_marker_count": args.min_before_marker_count,
            "min_delta_marker_count": args.min_delta_marker_count,
        ...
        "research_ranking": {
            ...
            "label": label,
            "interpretation": (
                "KEEP means the before/delta denominator scorer can verify selected no-order marker "
        ...
        "promotion_gate": {
            ...
            "status": "LEDGER_BEFORE_DELTA_MARKER_SCORER_ONLY_NOT_PROMOTION_EVIDENCE",
        },
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ledger-before-bucket", default="before_gte_1")
    parser.add_argument("--ledger-delta-bucket", default="delta_unknown")
    parser.add_argument("--min-before-marker-count", type=float, default=1.0)
    parser.add_argument("--min-delta-marker-count", type=float, default=1.0)
