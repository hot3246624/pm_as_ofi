#!/usr/bin/env python3
"""Run a local no-order residual guard grid for maker-shadow proxy inputs.

This script imports the research-only maker-shadow runner and evaluates bounded
residual-control variants on an existing local CSV.  It does not fetch public
...
KEEP_STATUS = "KEEP_NAGI_CE25_B27BC_RESIDUAL_GUARD_GRID_CANDIDATE_FOUND_RESEARCH_PROXY_NOT_READY"
BLOCKED_STATUS = "BLOCKED_NAGI_CE25_B27BC_RESIDUAL_GUARD_GRID_NO_PASS_NOT_READY"

GATE_PROFILES: dict[str, tuple[str, ...]] = {
    "all_gates": (),
    "primary_35_50": ("ce25_last60_35_50_primary",),
    "primary_50_65": ("ce25_last60_50_65_primary",),
    "primary_35_65": ("ce25_last60_35_50_primary", "ce25_last60_50_65_primary"),
    "overlay_20_35": ("ce25_last60_20_35_overlay",),
    "low_plus_35_50": ("ce25_last60_20_35_overlay", "ce25_last60_35_50_primary"),
}
...
def run_variant(rows: list[dict[str, str]], cfg: shadow.PipelineConfig) -> dict[str, Any]:
    ...
    return {
        **overall,
        "open_rejected_yes_first_suppressor": event_counts.get("open_rejected_yes_first_suppressor", 0),
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--residual-discount-s", type=float, action="append", default=[])
    ...
    p.add_argument("--include-yes-first-suppressor", action="store_true")
    p.add_argument("--include-residual-quarantine", action="store_true")
    p.add_argument("--include-coverage-profiles", action="store_true")
...
def main() -> int:
    ...
    residual_discounts = args.residual_discount_s or [15.0, 30.0]
    hard_timeouts = args.hard_timeout_s or [60.0, 90.0, 120.0, 180.0]
    pair_caps = args.pair_cost_cap or [0.95, 0.975, 0.995]
    results: list[dict[str, Any]] = []
    gate_profiles = GATE_PROFILES if args.include_coverage_profiles else {"all_gates": ()}
    ...
    for (
        ...
        hard_timeouts,
        pair_caps,
        bool_values(args.include_yes_first_suppressor),
        bool_values(args.include_residual_quarantine),
        gate_profiles,
        side_profiles,
    ):
        if hard_timeout_s < residual_discount_s:
            continue
        cfg = shadow.PipelineConfig(
            ...
            residual_discount_s=residual_discount_s,
            hard_timeout_s=hard_timeout_s,
            suppress_yes_first_open=suppress_yes,
        ...
        variant_id = (
            f"rd{int(residual_discount_s)}_ht{int(hard_timeout_s)}_"
            f"pc{pair_cost_cap:.3f}_yes{int(suppress_yes)}_q{int(quarantine)}_"
        ...
        row["passes_gate"] = (
            metric(row, "roi_proxy_conservative", -1.0) > 0
    ...
    results.sort(
        key=lambda row: (
            ...
            metric(row, "observed_markets", 0.0),
        ),
        reverse=True,
    )
    passing = [row for row in results if row["passes_gate"]]
    best_by_roi = max(results, key=lambda row: metric(row, "roi_proxy_conservative", -1.0), default=None)
    best_by_residual = min(results, key=lambda row: metric(row, "resid_rate", 1.0), default=None)
    ...
    decision = {
        ...
        "next_executable_action": (
            "review_best_residual_guard_variant_for_artifact_replay"
            if passing
            else "design_new_residual_model_or_require_private_maker_telemetry"
        ),
    }
