#!/usr/bin/env python3
"""Score symmetric activation event-lite summaries.

This scorer reads only runner summary/aggregate JSON outputs. It validates the
default-off contract, enabled schema, field contract, selected activation
denominators, source coverage, and aggregate parity. It does not fetch data,
start services, use SSH/shadow/live, or inspect events JSONL/raw stores.
"""
...
ARTIFACT = "xuan_symmetric_activation_summary_scorer"
SCHEMA_VERSION = "symmetric_activation_contract_summary_v1"
DEFAULT_SMOKE_ROOT = Path(
    "xuan_research_artifacts/xuan_symmetric_activation_runner_instrumentation_smoke_20260524T165500Z"
)
...
def summary_from_aggregate(aggregate: dict[str, Any]) -> dict[str, Any] | None:
    value = nested_get(aggregate, ["event_lite", "symmetric_activation_summary"])
...
def load_summary_files(summary_dir: Path, root: Path) -> tuple[list[dict[str, Any]], str | None]:
    ...
    for path in sorted(summary_dir.glob("*.summary.json")):
        ...
        if isinstance(obj, dict):
            summary = summary_from_aggregate(obj)
            if summary is None:
                summary = nested_get(obj, ["event_lite", "symmetric_activation_summary"])
...
def score(default_aggregate: dict[str, Any] | None, enabled_aggregate: dict[str, Any] | None, summary_rows: list[dict[str, Any]]) -> dict[str, Any]:
    blockers: list[str] = []
    checks: dict[str, Any] = {}
    ...
    blocked_count = nested_get(
        enabled_summary,
        [
            "candidate_count_by_status_reason_activation_bucket",
            "blocked|activation_opp_seen",
            "activation_required_missing_opp",
        ],
    )
    admitted_count = nested_get(
        enabled_summary,
        [
            "candidate_count_by_status_reason_activation_bucket",
            "admitted|candidate",
            "activation_required_activation_opp_age_1_5s",
        ],
    )
    blocked_count = float(blocked_count or 0.0)
    admitted_count = float(admitted_count or 0.0)
    checks["blocked_activation_denominator_positive"] = blocked_count >= 1.0
    checks["admitted_activation_denominator_positive"] = admitted_count >= 1.0
    if not checks["blocked_activation_denominator_positive"]:
        blockers.append("blocked_activation_denominator_missing")
    if not checks["admitted_activation_denominator_positive"]:
        blockers.append("admitted_activation_denominator_missing")

    blocked_source = nested_get(
        enabled_summary,
        [
            "source_sequence_presence_by_status_reason_activation_bucket",
            "blocked|activation_opp_seen|activation_required_missing_opp",
            "present",
        ],
    )
    admitted_source = nested_get(
        enabled_summary,
        [
            "source_sequence_presence_by_status_reason_activation_bucket",
            "admitted|candidate|activation_required_activation_opp_age_1_5s",
            "present",
        ],
    )
    checks["source_sequence_coverage_present"] = float(blocked_source or 0.0) >= 1.0 and float(admitted_source or 0.0) >= 1.0
    if not checks["source_sequence_coverage_present"]:
        blockers.append("source_sequence_coverage_missing")

    projected_pair_bucket = nested_get(
        enabled_summary,
        [
    ...
    checks["projected_pair_cost_bucket_present"] = float(projected_pair_bucket or 0.0) >= 1.0
    if not checks["projected_pair_cost_bucket_present"]:
        blockers.append("projected_pair_cost_bucket_missing")

    aggregate_total = nested_sum(enabled_summary.get("candidate_count_by_status_reason_activation_bucket"))
    summary_total = sum(nested_sum(row.get("candidate_count_by_status_reason_activation_bucket")) for row in summary_rows)
    checks["aggregate_parity"] = not summary_rows or abs(aggregate_total - summary_total) <= 1e-9
    if not checks["aggregate_parity"]:
    ...
    return {
        ...
        "blockers": blockers,
        "summary": {
            "blocked_activation_denominator": blocked_count,
            "admitted_activation_denominator": admitted_count,
            "aggregate_total": aggregate_total,
            "summary_file_total": summary_total,
...
def build_manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    decision_label = (
        "KEEP_SYMMETRIC_ACTIVATION_SUMMARY_SCORER_READY"
        if decision == "KEEP"
        else "UNKNOWN_SYMMETRIC_ACTIVATION_SUMMARY_SCORER_INPUTS_INSUFFICIENT"
    )
    return {
        "artifact": ARTIFACT,
        "schema_version": 1,
        ...
        "next_executable_action": (
            "Use this scorer on future allowlisted no-order pullbacks that include symmetric_activation_summary_v1; "
            "do not start no-order diagnostics from scorer readiness alone."
            if decision == "KEEP"
            else "Fix missing summary/aggregate fields before any no-order diagnostics."
        ),
        "research_ranking": {
