...
out_dir="${1:-$ROOT/xuan_research_artifacts/xuan_b27_dplus_source_opportunity_marker_summary_scorer_smoke_$ts}"
mkdir -p "$out_dir"
log="$out_dir/smoke.log"
: > "$log"

script="scripts/xuan_b27_dplus_source_opportunity_marker_summary_scorer.py"
fixture_root="${SOURCE_OPPORTUNITY_MARKER_FIXTURE_ROOT:-}"
if [[ -z "$fixture_root" ]]; then
  fixture_root="$(find "$ROOT/xuan_research_artifacts" -maxdepth 1 -type d -name 'xuan_b27_dplus_passive_passive_shadow_runner_source_opportunity_marker_smoke_*' | sort | tail -1)"
fi
if [[ -z "$fixture_root" || ! -d "$fixture_root" ]]; then
  echo "missing source-opportunity marker smoke fixture root" >&2
  exit 2
fi
...
old_no_order_root="$ROOT/xuan_research_artifacts/xuan_b27_dplus_shadow_review_micro_deficit_driver_20260523T005615Z/remote_clean/output"
...
if [[ -d "$old_no_order_root" && -f "$old_no_order_root/aggregate_report.json" ]]; then
  ...
  set -e
  if [[ "$old_rc" -ne 3 ]]; then
    echo "expected old no-order fixture without source-opportunity marker summary to return 3, got $old_rc" >&2
    exit 1
  fi
fi
...
checks = {
    "py_compile": True,
    "enabled_keep": enabled["decision_label"] == "KEEP_SOURCE_OPPORTUNITY_MARKER_DENOMINATOR_SCORER_READY",
    "enabled_aggregate_parity": enabled["aggregate_parity"]["passed"] is True,
    "enabled_marker_count_nonzero": enabled["marker_denominator"]["strict_micro_deficit_marker_total"] >= 1,
    "enabled_status_reason_context": "admitted|candidate" in enabled["marker_status_reason_context"],
    ...
    "default_off_unknown": default_rc == 3 and default_off["decision_label"] == "UNKNOWN_SOURCE_OPPORTUNITY_MARKER_SUMMARY_INPUTS_INSUFFICIENT",
    "default_off_missing_summary_blocker": "summary_source_opportunity_marker_missing" in default_off["blockers"],
    "old_no_order_unknown_if_available": True,
}
if old_score is not None:
    checks["old_no_order_unknown_if_available"] = (
        old_rc == 3
        and old_score["decision_label"] == "UNKNOWN_SOURCE_OPPORTUNITY_MARKER_SUMMARY_INPUTS_INSUFFICIENT"
        and "summary_source_opportunity_marker_missing" in old_score["blockers"]
    )

if not all(checks.values()):
    failed = [key for key, value in checks.items() if not value]
    raise AssertionError(f"source-opportunity marker scorer smoke failed checks: {failed}")

manifest = {
    "artifact": "xuan_b27_dplus_source_opportunity_marker_summary_scorer_smoke",
    "created_utc": out_dir.name.rsplit("_", 1)[-1],
    "decision": "KEEP",
    "decision_label": "KEEP_SOURCE_OPPORTUNITY_MARKER_SUMMARY_SCORER_SMOKE_PASS",
    "inputs": {
    ...
    "research_ranking": {
        ...
        "interpretation": "The scorer can validate source-opportunity marker summary fields/parity, detect exact frozen marker denominators, and fail closed on default-off or old pullbacks without the new summary."
    },
    "promotion_gate": {
        "passed": False,
    ...
    "next_action": "Use this local scorer after an explicitly approved source-opportunity marker no-order pullback, or keep working locally on scorer/spec only."
}
