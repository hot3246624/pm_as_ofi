...
ARTIFACT="${ARTIFACT:-$ROOT/xuan_research_artifacts/xuan_b27_dplus_selected_seed_ledger_context_export_smoke_${TS}}"
...
python3 scripts/xuan_b27_dplus_candidate_pipeline_state_machine_rerun.py \
  ...
  --selected-seed-ledger-context-output "$ARTIFACT/export/selected_seed_ledger_contexts.csv" \
  --selected-seed-ledger-context-profile control_seed_offset_max_120 \
  --selected-seed-ledger-context-sample-cap 10 \
  --selected-seed-ledger-context-per-day-cap 10 \
  --selected-seed-ledger-context-selection-mode residual_tail >/dev/null
...
with (artifact / "export" / "selected_seed_ledger_contexts.csv").open(newline="") as handle:
...
checks = {
    "default_export_disabled": default_manifest["selected_seed_ledger_context_export"]["enabled"] is False,
    "export_enabled": export_manifest["selected_seed_ledger_context_export"]["enabled"] is True,
    ...
    "pre_seed_inventory_context_present": (
        float(residual_row.get("pre_seed_same_qty") or 0.0) == 0.0
...
manifest = {
    "artifact": "xuan_b27_dplus_selected_seed_ledger_context_export_smoke",
    "schema_version": "selected_seed_ledger_context_export_smoke_v1",
    "decision": "KEEP" if all(checks.values()) else "BLOCKED",
    "label": (
        "KEEP_SELECTED_SEED_LEDGER_CONTEXT_EXPORT_SMOKE_PASS"
        if all(checks.values())
        else "BLOCKED_SELECTED_SEED_LEDGER_CONTEXT_EXPORT_SMOKE_FAIL"
    ),
    "checks": checks,
    "selected_seed_ledger_context_summary": export_manifest["selected_seed_ledger_context_export"]["summary"],
    "selected_seed_ledger_context_csv": str(artifact / "export" / "selected_seed_ledger_contexts.csv"),
    "side_effects": {
