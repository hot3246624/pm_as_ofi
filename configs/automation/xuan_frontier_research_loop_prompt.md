...
Recommended schedule if manually re-enabled: `FREQ=HOURLY;INTERVAL=1`
...
This local archive loop is intentionally PAUSED because Codex cron-generated threads are currently not reliably hidden by archive directives in the sidebar. Keep the canonical prompt for manual/on-demand use, but do not run it on a schedule unless the app archive behavior is fixed or the user explicitly requests visible local archive runs.
...
Strategic state: b27/D+ two-sided passive BUY from public SELL flow is the leading core; xuan is the completion/inventory lifecycle benchmark; rwo/d189 is tail/ITM control. Metrics priority: PnL/stress first, supported by residual rate, cycles/market, and pair cost. Current strongest implementable-size D+ result from interactive/remote work: t10/imb8/sv0950 fill_haircut=0.20 over 2026-05-02..2026-05-13 had net_pair_cost=0.922802, cycles=11.3190/market, qty_residual=3.8242%, PnL=+4985.76, ROI=6.9520%, stress100_worst=+2410.78. 2026-05-13 holdout stayed positive; best stress row was fh020/imb6/sv0960 with net_pair_cost=0.922410, cycles=10.3681/market, qty_residual=3.4398%, PnL=+345.25, ROI=7.1615%, stress100_worst=+175.70. Xuan public benchmark: pair_cost about 0.9783, cycles about 5.89/market, residual_market_rate about 11.08%, ROI about 2.22%.

Code state: origin/codex/xuan-frontier has dry-run-only D+ min-order profile PM_PGT_SHADOW_PROFILE=dplus_minorder_v1 and per-side public SELL snapshots. The profile is not deployed and not production/live enabled. Highest-value local loop work if manually re-enabled: inspect local artifacts and code state for D+ min-order profile follow-ups, especially missing diagnostics/recorder fields, tests, verifier/shadow readiness notes, and patch proposals.

STRICT AUTO-ARCHIVE RULE: If re-enabled and this local archive run has no new local patch proposal, no decision request, no data-source request, no failed local run requiring main-thread action, and no material strategy finding from already-local artifacts, final output exactly and only: ::archive{reason="routine xuan frontier checkpoint"}. Do not output summaries, bullets, status text, "quiet archive", or "no changes" for routine runs. For material findings, keep visible final under 6 lines with exact artifact paths.
```
