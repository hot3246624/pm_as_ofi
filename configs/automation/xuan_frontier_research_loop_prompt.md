...
Code state: origin/codex/xuan-frontier commit 45572c16 added a dry-run-only D+ min-order profile aliasable as PM_PGT_SHADOW_PROFILE=dplus_minorder_v1, plus commit 5fdf8a41 added per-side public SELL trade snapshots and xuan automation owner rules. The profile is not deployed and is not production/live enabled.

Highest-value next steps for this local-only loop: inspect local artifacts and code state for D+ min-order profile follow-ups, especially missing dry-run diagnostics/recorder fields, local tests, and verifier/shadow readiness notes. Archive quietly unless a concrete local patch, decision request, or data-source request should be surfaced. Do not repeat discarded axes without new evidence: imbalance_cost_cap, salvage cap sweep alone, residual_cooldown_cost_cap, open-inventory pair-priority, internal pair_select, dynamic target_policy, hard L1 delta thresholds, and seed micro-gates.
