...
- Runner: `scripts/xuan_b27_dplus_compliant_metrics_runner.py`
...
The new runner is not the old completion-only probe. It reads:
...
```bash
python3 scripts/xuan_b27_dplus_compliant_metrics_runner.py \
  --strict-labels 20260509 \
  --completion-labels 20260509 \
  --max-strict-candidates-per-plan 1000
```
