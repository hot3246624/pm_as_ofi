...
| A4 | first fill 捕获 | `seed_exposed_fill_ratio` 持续提升；短样本不作为硬门槛 | 最新连续 2 个 active episode 均有 first fill；总样本仍少 | 观察中 | 至少再收集 10-20 轮 |
...
| A7 | taker-close SLA | shadow-only；触发后 `dispatch_taker_close > 0`，first fill 到 cover p90 <= 100s | 最新 2 个 active episode：`dispatch_taker_close=3`，`p90_first_completion_delay_s=81.115` | 通过 | 继续累计样本 |
...
| A9 | pair cost | 中位 `summary_pair_cost <= 1.00`，理想接近 0.99；不得系统性 >1.01 | 最新 2 个 active episode 中位 `summary_pair_cost=1.0100`，闭合质量偏保守 | 观察中 | 累计样本后决定是否放慢/收紧 taker-close |
...
## 最新聚合：`btc-updown-5m-1777468200` 到 `btc-updown-5m-1777468800`

样本数：2 个 active episode
...
- `total_dispatch_taker_close=3`
- `first_completion_delay_s_median=45.065`
- `p90_first_completion_delay_s=81.115`
- `merge_requested_first_rel_s_median=-24.893`
- `redeem_requested_first_rel_s_median=46.538`
...
- `summary_pair_cost=1.01` 是当前主要质量问题；如果长样本继续贴上限，需要收紧 taker-close 或延后 repair。
