...
| A4 | first fill 捕获 | `seed_exposed_fill_ratio` 持续提升；短样本不作为硬门槛 | 最新连续 3 个 active episode 均有 first fill | 观察中 | 至少再收集 10-20 轮 |
| A5 | same-side add | `same_side_add_qty_ratio` 目标约 0.05-0.15，且 `MAX_SAME_SIDE_RUN=1` | 最新 3 个 active episode 中位 `0.0941`，接近 xuan 目标 `0.105`；已修正报表 ratio 口径 | 通过 | 防止多次 same-side add 回归 |
...
| A7 | taker-close SLA | shadow-only；触发后 `dispatch_taker_close > 0`，first fill 到 cover p90 <= 100s | 最新 3 个 active episode：`dispatch_taker_close=5`，`p90_first_completion_delay_s=90.127` | 通过 | 继续累计样本 |
...
| A9 | pair cost | 中位 `summary_pair_cost <= 1.00`，理想接近 0.99；不得系统性 >1.01 | 最新 3 个 active episode 中位 `summary_pair_cost=1.0100`，最新单轮降至 `1.0091`；闭合质量仍偏保守 | 观察中 | 累计样本后决定是否放慢/收紧 taker-close |
...
## 最新聚合：`btc-updown-5m-1777468200` 到 `btc-updown-5m-1777479000`

样本数：3 个 active episode
...
- `total_dispatch_taker_close=5`
- `same_side_add_qty_ratio_median=0.0941`
- `first_completion_delay_s_median=90.125`
- `p90_first_completion_delay_s=90.127`
- `merge_requested_first_rel_s_median=-24.878`
