...
最新 close-only boundary 数据集评估基线，已包含 HYPE/XRP wide-spread near-flat guardrail、DOGE last-before guardrail，并已修正 evaluator，使其与运行时的 HYPE `close_only_fallback`、BNB/SOL close-only filter reason、BNB/SOL strict source fallback 保持一致：

| 样本段 | ok | side | filtered | missing | mean_bps | max_bps |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| latest test | 253 | 0 | 24 | 14 | 1.045063 | 6.801403 |
| latest train | 668 | 0 | 76 | 59 | 0.873134 | 4.413691 |
...
- G3：latest train max 已降到 `4.413691bps`；latest test `max_bps=6.801403` 仍超出 5bps，剩余瓶颈集中在 HYPE outlier。
...
| T+2h | 汇总 20+ 新轮次 | `20260429_220724` 已暴露 DOGE 高 spread outlier，guardrail 已补齐，需重启 challenger 验证 |
...
1. 重建 release 并重启 challenger，让 DOGE guardrail 生效；目标 rolling 100 accepted side=0。
2. 定向分析 HYPE `max_bps=6.801403` outlier；当前最强候选是 `bybit+okx+hyperliquid` 的 `twap_pre_5000`，但需要新 unseen 验证后才可替换主规则。
3. 继续观察 SOL/BNB strict source fallback 在新 unseen 中是否保持 side=0；任何 side mismatch 立即回滚为 missing。
4. 下一次建议在重启后 30 分钟复盘；如果 DOGE 不再出现 >5bps accepted，再集中处理 HYPE primary/fallback 精度。
