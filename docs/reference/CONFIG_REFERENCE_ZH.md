...
| `xuan_ladder_v1` | 最新 xuan public 行为 shadow 近似 + profit guard | 开盘后 4s 开始；收盘前 25s 停新 first leg；seed pair cap `1.040`；completion cap 默认守 `1.000`；只有真实 surplus repair budget 可最多放宽到 `1.030`；亏损闭合后新 first leg 启用 breakeven-path brake；taker-close cap `1.000`；clip 随轮内时间梯度变化 |
