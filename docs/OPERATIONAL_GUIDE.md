...
│  - OFI Toxicity (per-side kill + recovery hold)               │
│  - ACTION: unhealthy side target=0.0 (Cancel)                 │
├─────────────────────────────────────────────────────────────┤
...
│  - Inventory gate split: Provide(net+side), Hedge(net-only)   │
├─────────────────────────────────────────────────────────────┤
...
**Lead-Lag 逻辑**：YES 和 NO 是互补的（YES + NO = $1），但执行层采取“按侧熔断”。有毒侧先撤单并暂停，该侧恢复后再进入恢复冷却窗口，另一侧可在门禁允许时继续工作。
...
**避免**：
- 结果即将公布（避免在开盘后超过 `PM_ENTRY_GRACE_SECONDS` 的窗口盲目入场）
- 极低流动性（单次成交 < 1 USDC）
...
> 注：动态计算是**显式启用**。仅在配置 `PM_BID_PCT` / `PM_NET_DIFF_PCT` 时，系统才会按余额下调；不配置时保持静态值。
...
```bash
# 设置 slug 为前缀模式：
POLYMARKET_MARKET_SLUG=btc-updown-5m  # 自动轮换 5 分钟 BTC 涨跌市场
...
```bash
# 必填（实盘）
POLYMARKET_PRIVATE_KEY=...             # 签名私钥（不带 0x）
POLYMARKET_FUNDER_ADDRESS=0x...        # 资金地址（可与签名地址相同）
POLYMARKET_MARKET_SLUG=btc-updown-5m   # 市场前缀或完整 slug（或改用 SYMBOL+TIMEFRAME）
...
PM_MAX_POS_PCT=0.0            # 0=关闭动态 gross（静态优先）
...
PM_OFI_TOXICITY_THRESHOLD=300 # 毒性阈值（adaptive=true 时作为基准/回退值）
PM_OFI_HEARTBEAT_MS=200       # OFI 心跳间隔
PM_OFI_ADAPTIVE=true          # 启用自适应阈值
PM_OFI_ADAPTIVE_K=3.0         # 自适应系数（均值+Kσ）
PM_OFI_ADAPTIVE_MIN=120.0     # 自适应阈值下限
PM_OFI_ADAPTIVE_MAX=0.0       # 自适应阈值上限（0=关闭硬上限）

# 市场控制
PM_TICK_SIZE=0.01             # 报价最小单位
PM_ENTRY_GRACE_SECONDS=30     # 新市场开盘后允许入场窗口（秒）
```
