...
impl PathEvaluator {
    ...
    fn dfs_find_paths_states(
        ...
        for pool in self.pools.values() {
            if visited_pools.contains(&pool.address) { continue; }
            let next_token = if pool.token0 == current_token { pool.token1 }
                             else if pool.token1 == current_token { pool.token0 }
                             else { continue };

            visited_pools.insert(pool.address);
            current_path.push(pool.clone());
            self.dfs_find_paths_states(next_token, end_token, current_path, all_paths, visited_pools, hops_left - 1);
            current_path.pop();
    ...
    pub async fn evaluate_path(
        ...
        for hop in path.hops.iter_mut() {
            ...
            let sim_result = self.simulate_hop(pool_state, current_amount, hop.token_in).await?;
        ...
        match &pool_state.amm_data {
            AmmData::V3(data) => {
                ...
                let price_ratio = if zero_for_one {
                    ...
                    let price_scaled = (sqrt_price_scaled * sqrt_price_scaled) / U256::from(10).pow(U256::from(24));
                    price_scaled
                } else {
                    // token1 -> token0: 反向计算
                    let sqrt_price_scaled = sqrt_price / U256::from(10).pow(U256::from(12));
                ...
                debug!("价格影响计算: amount_in_after_fee={}, liquidity={}, liquidity_ratio={}", 
                ...
                let adjusted_price_ratio = (price_ratio * price_impact_multiplier) / U256::from(10000);
                
                // 计算输出数量
                let amount_out = (amount_in_after_fee * adjusted_price_ratio) / U256::from(10).pow(U256::from(18));
    ...
    pub async fn simulate_swap(&self, swap: &Swap, pool_state: &PoolState) -> Result<SwapSimulationResult> {
        let (token_in, amount_in) = match swap {
            Swap::V2 { path, amount_in, .. } => (path[0], *amount_in),
            Swap::V3 { token_in, amount_in, .. } => (*token_in, *amount_in),
        };
